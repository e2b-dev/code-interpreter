...
sandboxTest('js context', async ({ sandbox }) => {
  await sandbox.runCode('const z = 1', { language: 'tslab' })
  const result = await sandbox.runCode('z', { language: 'tslab' })
  expect(result.results[0].text).toEqual('1')
})

sandboxTest('js cwd', async ({ sandbox }) => {
  const result = await sandbox.runCode('process.cwd()', { language: 'tslab' })
